...
@Component({
  selector: 'app-recovery-docente',
  templateUrl: './recovery-docente.page.html',
  styleUrls: ['./recovery-docente.page.scss'],
  standalone: true,
  imports: [IonicModule, FormsModule, CommonModule, ReactiveFormsModule, RouterModule]
})
export class RecoveryDocentePage implements OnInit {
  ...
  toClick() {
    if(this.loginFormAlumno.valid){
      ...
      this.router.navigate( ['docente'], { queryParams:{username, newpassword} });
